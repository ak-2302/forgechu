...
export async function processDueReminders(env: Env) {
  ...
  for (const reminder of dueReminders.results) {
    await sendReminderPush(env, reminder.user_id, "Reminder", reminder.body);
    if (reminder.repeat_interval <= 0) {
      await env.DB.prepare(
        "UPDATE reminder_notes SET is_done = 1, last_remind_at = ? WHERE id = ?",
      )
        .bind(now, reminder.id)
        .run();
    } else {
      const nextRemindAt = now + reminder.repeat_interval;
      await env.DB.prepare(
